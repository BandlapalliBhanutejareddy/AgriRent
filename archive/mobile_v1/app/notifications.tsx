  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity 
      style={[
        styles.card, 
        { 
          backgroundColor: item.read ? theme.card + '50' : theme.card, 
    ...
    >
      <View style={styles.iconContainer}>
          <View style={[styles.iconBg, { backgroundColor: item.read ? theme.border : theme.primary + '15' }]}>
            {getIcon(item.type || '')}
          </View>
          {!item.read && <View style={[styles.unreadBadge, { backgroundColor: theme.primary }]} />}
      </View>
      
      <View style={styles.content}>
        <View style={styles.headerRow}>
          <Text style={[typography.title, { color: theme.text, fontWeight: item.read ? '600' : '800' }]} numberOfLines={1}>
            {item.title}
          </Text>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={[typography.h2, { color: theme.text }]}>{t('notifications')}</Text>
        <TouchableOpacity>
            <Text style={[typography.label, { color: theme.primary }]}>{t('clear_all')}</Text>
        </TouchableOpacity>
      </View>
