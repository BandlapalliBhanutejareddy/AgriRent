...
function MarketplaceScreen() {
  ...
  // Sync advanced filters when global categoryFilter changes
  useEffect(() => {
    if (categoryFilter && categoryFilter !== 'ALL') {
      const formattedCategory = categoryFilter.charAt(0) + categoryFilter.slice(1).toLowerCase();
      setAdvancedFilters(prev => ({ ...prev, category: formattedCategory }));
    } else {
  ...
  const fetchEquipment = async () => {
    try {
      // Fetching nearby equipment for Nashik (mock user location)
      const response = await api.get('/equipment/nearby?lat=19.9975&lng=73.7898&radius=100');
  ...
  const handleVoiceSearch = () => {
    Alert.alert(
      "Listening...",
      "Speak now to search equipment (Mocked for Demo)",
      [
        { text: "Cancel", style: "cancel" },
        { 
          text: "Say 'Tractor'", 
          onPress: () => {
            setLocalSearch("Tractor");
            setSearchQuery("Tractor");
          }
        }
      ]
    );
