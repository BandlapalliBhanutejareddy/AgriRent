...
async function generateCrossExcelReport() {
  const reportsDir = path.join(__dirname, '../../reports/cross-platform');
  ...
  const sheet = workbook.addWorksheet('Cross-Platform Results');
  ...
  const outputPath = path.join(reportsDir, 'Cross_Platform_Test_Report.xlsx');
  await workbook.xlsx.writeFile(outputPath);
  console.log(`Excel report generated at: ${outputPath}`);
}

if (require.main === module) {
  generateCrossExcelReport().catch(console.error);
}

module.exports = { generateCrossExcelReport };
